...
const classStyle: Record<string, { border: string; badge: string }> = {
  CRITICAL: { border: 'border-red-800',    badge: 'bg-red-900/40 text-red-400' },
  ERROR:    { border: 'border-orange-800', badge: 'bg-orange-900/40 text-orange-400' },
  WARNING:  { border: 'border-yellow-800', badge: 'bg-yellow-900/40 text-yellow-400' },
  INFO:     { border: 'border-blue-800',   badge: 'bg-blue-900/40 text-blue-400' },
}
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">
          Alarme
          {active.length > 0 && (
            <span className="ml-2 text-sm bg-red-900/40 text-red-400 border border-red-800 px-2 py-0.5 rounded-full">
              {active.length} aktiv
            </span>
          )}
        </h1>
      ...
      {alarms.length === 0 ? (
        <div className="text-center py-16 space-y-2">
          ...
          <p className="text-green-400 font-medium">Keine aktiven Alarme</p>
          <p className="text-gray-600 text-sm">System läuft normal.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {alarms.map((a: any) => {
            ...
            return (
              <div key={a.id} className={`rounded-xl p-4 border bg-gray-900 ${s.border}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-xs font-mono text-gray-400">{a.alarm_code}</span>
                      <span className={`text-xs px-2 py-0.5 rounded ${
                        a.status === 'ACTIVE' ? 'bg-red-900/30 text-red-400' :
                  ...
                  <div className="flex gap-2 shrink-0">
                    {a.status === 'ACTIVE' && (
                      <button
                        ...
                        className="text-xs px-3 py-1.5 rounded-lg bg-yellow-900/30 text-yellow-400 border border-yellow-800 hover:bg-yellow-900/50 disabled:opacity-50 transition-colors"
                      >
                    ...
                    {a.status !== 'CLOSED' && (
                      <button
                        onClick={() => close(a.id)}
                        disabled={working === a.id}
                        className="text-xs px-3 py-1.5 rounded-lg bg-gray-800 text-gray-400 border border-gray-700 hover:bg-gray-700 disabled:opacity-50 transition-colors"
                      >
                        Schließen
                      </button>
                    )}
