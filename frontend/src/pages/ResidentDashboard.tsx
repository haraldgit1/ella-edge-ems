  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <select
          className="ml-auto bg-gray-900 border border-gray-700 rounded-lg px-3 py-1.5 text-sm text-gray-200 focus:outline-none focus:border-gray-500"
          value={selectedId}
      ...
      {data?.current_power_w != null && (
        <div className="bg-gray-900 rounded-xl p-4 border border-gray-800 flex items-center gap-4">
          ...
          <span className="text-sm text-gray-400">Aktueller Verbrauch</span>
          <span className="font-bold text-white">{Math.round(data.current_power_w)} W</span>
      ...
      <div className="bg-gray-900 rounded-xl p-5 border border-gray-800 space-y-4">
        <p className="text-sm font-medium text-gray-300">Heute</p>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div className="bg-gray-800 rounded-lg p-3">
            ...
            <p className="text-lg font-bold">{today?.consumption_kwh ?? '—'} <span className="text-sm font-normal text-gray-400">kWh</span></p>
          </div>
          <div className="bg-green-900/20 border border-green-900/40 rounded-lg p-3">
            ...
            <p className="text-lg font-bold text-green-400">{today?.local_kwh ?? '—'} <span className="text-sm font-normal text-gray-400">kWh</span></p>
          </div>
          <div className="bg-gray-800 rounded-lg p-3">
            <p className="text-xs text-gray-500 mb-1">Ergänzung aus Netz</p>
            <p className="text-lg font-bold text-gray-300">{today?.grid_kwh ?? '—'} <span className="text-sm font-normal text-gray-400">kWh</span></p>
          </div>
      ...
      <div className="bg-gray-900 rounded-xl p-5 border border-gray-800 space-y-3">
        <p className="text-sm font-medium text-gray-300">Laufender Monat</p>
        ...
        <div className="grid grid-cols-3 gap-3 text-sm">
          ...
          <div className="text-center">
            <p className="text-xs text-green-500">Lokal</p>
            <p className="font-bold text-green-400">{month?.local_kwh ?? '—'} kWh</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-gray-500">Netz</p>
            <p className="font-bold text-gray-300">{month?.grid_kwh ?? '—'} kWh</p>
          </div>
        </div>
      </div>
