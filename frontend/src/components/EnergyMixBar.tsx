    ...
    <div className="space-y-1">
      {label && (
        <div className="flex justify-between text-xs text-gray-400">
          <span className="text-green-400">Lokaler Gemeinschaftsstrom {pct}%</span>
          <span className="text-gray-400">Netzstrom {gridPct}%</span>
        </div>
      )}
      <div className="h-6 rounded-full overflow-hidden bg-gray-700 flex">
        {pct > 0 && (
          <div
        ...
        {gridPct > 0 && (
          <div
            ...
            style={{ width: `${gridPct}%` }}
          >
            {gridPct >= 15 && <span className="text-xs text-gray-300 font-medium">{gridPct}%</span>}
          </div>
        )}
      </div>
    </div>
