      ...
      <div className="flex flex-col lg:flex-row gap-3 flex-1 min-h-0">
        ...
        <div className="flex flex-col gap-3 lg:w-96 shrink-0">
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-4 space-y-3">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Filter</h2>
            <div className="grid grid-cols-2 gap-2">
              <input type="text" placeholder="CID" value={cid} onChange={e => setCid(e.target.value)}
                className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm placeholder-gray-600 focus:outline-none focus:border-purple-600" />
              ...
              <div>
                <label className="text-[10px] text-gray-600 block mb-0.5">Von</label>
              ...
              <div>
                <label className="text-[10px] text-gray-600 block mb-0.5">Bis</label>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={loadList}
                className="ml-auto px-3 py-1 text-xs rounded-lg bg-gray-800 text-gray-300 border border-gray-700 hover:bg-gray-700 transition-colors">
                Aktualisieren
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-auto bg-gray-900 rounded-xl border border-gray-800 min-h-0">
            <table className="w-full text-xs">
              ...
              <tbody>
                {list.length === 0 && (
                  <tr><td colSpan={3} className="text-center text-gray-600 py-6">Keine Einträge</td></tr>
                )}
                {list.map(e => (
                  <tr key={e.id} onClick={() => selectRow(e)}
                    ...
                    <td className="px-3 py-2 font-mono text-[10px] text-gray-400 whitespace-nowrap">
                      {fmtTs(e.received_at)}
                    </td>
                    <td className="px-2 py-2 text-gray-300 font-mono">{e.cid ?? '—'}</td>
        ...
        <div className="flex-1 min-w-0 overflow-auto bg-gray-900 rounded-xl border border-gray-800 p-5">
          ...
          {!form ? (
            <p className="text-gray-600 text-sm text-center mt-16">Eintrag aus der Liste wählen</p>
          ) : (
            <div className="space-y-6">

              {/* Readonly fields */}
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 p-4 bg-gray-800/40 rounded-xl">
                <div>
                  <div className="text-[10px] text-gray-500 uppercase tracking-wide mb-1">Empfangen</div>
                  <div className="text-sm font-mono text-gray-300">{fmtTs(form.received_at)}</div>
                </div>
                <div>
                  <div className="text-[10px] text-gray-500 uppercase tracking-wide mb-1">Quell-IP</div>
                  <div className="text-sm font-mono text-gray-300">{form.source_ip ?? '—'}</div>
                </div>
                <div>
                  <div className="text-[10px] text-gray-500 uppercase tracking-wide mb-1">Status</div>
                  <span className={`text-sm font-bold ${form.status === 'OK' ? 'text-green-400' : 'text-red-400'}`}>
                    {form.status}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-4">
                  <h3 className="text-xs font-semibold text-amber-400 uppercase tracking-widest border-b border-gray-800 pb-2">Push-Daten</h3>
                  <div className="space-y-1">
                    ...
                    <input value={form.cid ?? ''} onChange={e => upd({ cid: e.target.value || null })}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm text-gray-100 focus:outline-none focus:border-purple-600" />
                  </div>
                  <div className="space-y-1">
                    ...
                    <input value={form.pact_str ?? ''} onChange={e => upd({ pact_str: e.target.value || null })}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm font-mono text-gray-100 focus:outline-none focus:border-purple-600" />
                  </div>
                  <div className="space-y-1">
                ...
                <div className="space-y-4">
                  <h3 className="text-xs font-semibold text-blue-400 uppercase tracking-widest border-b border-gray-800 pb-2">Auflösung + Fehler</h3>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea value={form.error_msg ?? ''} onChange={e => upd({ error_msg: e.target.value || null })}
                      rows={4}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm text-gray-100 focus:outline-none focus:border-purple-600 resize-none" />
                  </div>
                </div>
              </div>
            </div>
